...
@section('left-panel')
    @include('partials.admin.left-panel', ['page'=>'admin.rap', 'subpage'=>'admin.rap.manage.role'])
@endsection
...
@section('content')
    <style>
        ...
        #role-members-search-result-box {
        ...
        .role-member-search-user, .selected-role-member-to-get-role {
            padding: 8px;
    ...
    <div class="flex align-center space-between top-page-title-box">
        ...
        <div class="flex align-center height-max-content">
            ...
            <div class="flex align-center">
                <span class="fs13 bold">{{ __('Manage a role') }}</span>
            </div>
        </div>
    </div>
    ...
    <div id="create-role-viewer" class="global-viewer full-center none">
        ...
        <div class="global-viewer-content-box viewer-box-style-1" style="width: 600px;">
            ...
            <div class="viewer-scrollable-box scrolly" style="padding: 14px; max-height: 430px">
                <!-- messages and inputs -->
                <input type="hidden" id="existing-roles-names" value="{{ $roles->pluck('role')->implode(',') }}" autocomplete="off">
                <input type="hidden" id="existing-roles-slugs" value="{{ $roles->pluck('slug')->implode(',') }}" autocomplete="off">
                
                <input type="hidden" id="role-name-already-exists" value="Role name already exists" autocomplete="off">
                <input type="hidden" id="role-slug-already-exists" value="Role slug already exists" autocomplete="off">
                <input type="hidden" id="role-name-required" value="Role name is required" autocomplete="off">
                <input type="hidden" id="role-slug-required" value="Role slug is required" autocomplete="off">
                <input type="hidden" id="role-description-required" value="Role description is required" autocomplete="off">


                <div class="section-style fs13 lblack mb8">
                    <p class="no-margin">Here you can create a new role and attach it to users. But before that, you have to consider the following points</p>
                    <div class="ml8 mt8">
                        <div class="flex mt4">
                            <div class="fs10 mr8 mt4 gray">•</div>
                            <p class="no-margin" style="line-height: 1.5">After creating the role, it will take the <strong>lowest priority</strong>. (means higher priority column value). You can change its priority after by going to manage section of that role.</p>
                        </div>
                        <div class="flex mt4">
                            <div class="fs10 mr8 mt4 gray">•</div>
                            <p class="no-margin" style="line-height: 1.5">Role name and slug <strong>must be unique</strong>. (You cannot have two roles with the same name or slug).</p>
                        </div>
                    </div>
                </div>

                <p class="fs13 no-margin mb4">The following roles are ordered by priority. The created role will come to the last place with lower priority. However you can update it after If you will.</p>
                <div class="flex align-center">
                    <p class="my8 bold forum-color mr8">Existing roles :</p>
                    <div class="relative">
                        <div class="button-with-suboptions button-style-2">
                            <span>see roles</span>
                        ...
                        <div class="suboptions-container suboptions-container-right-style scrolly" style="left: 0; max-height: 260px">
                            @foreach($roles as $r)
                            <div class="simple-suboption" style="max-width: 270px; cursor: default;">
                                <span class="fs13 bold">{{ $r->role }}</span>
                                <p class="fs12 gray my4">{{ $r->descriptionslice }}</p>
                            </div>
                            @endforeach
                        </div>
                    </div>
                ...
                <div>
                    <div class="flex align-center">
                        ...
                        <p class="my8 bold forum-color">Role Informations</p>
                    </div>
                    ...
                    <div class="mb8">
                        <label for="create-role-name-input" class="flex align-center bold forum-color fs13">{{ __('Name') }}<span class="ml4 err red none fs12">*</span></label>
                        <p class="no-margin fs12 mb2 gray">Role name should not contain commas(,) because we use it as separator for existing names</p>
                        <input type="text" autocomplete="off" class="input-style-1 full-width" id="create-role-name-input" placeholder="Role name" style="padding: 8px 10px">
                    </div>
                    <div class="mb8">
                        <label for="create-role-slug-input" class="flex align-center bold forum-color fs13">{{ __('Slug') }}<span class="ml4 err red none fs12">*</span></label>
                        <p class="no-margin fs12 mb2 gray">Role slug also should not contain commas(,) for the same reason as names</p>
                        <input type="text" autocomplete="off" class="input-style-1 full-width" id="create-role-slug-input" placeholder="Role slug" style="padding: 8px 10px">
                    </div>
                    <div class="mb8">
                        <label for="create-role-description-input" class="flex align-center bold forum-color mb4 fs13">{{ __('Description') }}<span class="ml4 err red none fs12">*</span></label>
                        <textarea id="create-role-description-input" class="styled-textarea fs14"
                ...
                <p class="no-margin mb4 bblack">Please type <strong>{{ auth()->user()->username }}::create-role</strong> to confirm.</p>
                <div>
                    <input type="text" autocomplete="off" class="full-width input-style-1" id="create-role-confirm-input" style="padding: 8px 10px" placeholder="confirmation">
                ...
                <div class="flex" style="margin-top: 12px">
                    <div class="flex align-center full-width">
                        <div id="create-role-button" class="disabled-green-button-style green-button-style full-center full-width">
    ...
    <div id="admin-main-content-box">
        @if(is_null($role))
            <div class="flex align-center">
                <h2 class="no-margin fs20 mb2 lblack">Select role to manage</h2>
                <div class="gray height-max-content fs10 bold" style="margin: 0 12px">•</div>
                @if($canadd)
                <div class="green-button-style width-max-content flex align-center my4 open-create-role-dialog">
            ...
            <p class="no-margin fs13 lblack mb4">Select one of the following existing roles to manage. The roles are ordered by priority from higher to lower priority (from left to right).</p>
            <div class="flex justify-center flex-wrap" style="margin: 12px 0">
                @foreach($roles as $r)
                <a href="{{ route('admin.rap.manage.role') . '?roleid=' . $r->id }}" class="rounded-entity full-center flex-column lblack no-underline mx8" style="width: 80px; height: 80px;">
                    <span class="bold text-center">{{ $r->role }}</span>
                    <span class="fs10">click to manage</span>
                </a>
                @endforeach
            </div>
        @else
            ...
            <div class="viewer-box-style-1" style="width: 600px;">
                <div class="flex align-center space-between light-gray-border-bottom" style="padding: 14px;">
                    <div class="flex align-center">
                        ...
                        <span class="fs20 bold forum-color">Grant "{{ $role->role }}" role to users</span>
                    </div>
                    <div class="pointer fs20 close-global-viewer unselectable">✖</div>
                </div>
                <div class="viewer-scrollable-box scrolly" style="padding: 14px; max-height: 430px">
                    ...
                    <div class="section-style fs13 lblack mb8">
                        <p class="no-margin">Here you can create grant "{{ $role->role }}" role to members. Before proceding this process, consider the following points</p>
                        <div class="ml8 mt8">
                            <div class="flex mt4">
                                <div class="fs10 mr8 mt4 gray">•</div>
                                <p class="no-margin" style="line-height: 1.5">Once the selected member(s) get the role, he will <strong>acquired all permissions in that role</strong> that allow him to perform all activities allowed by all its associated permissions.</p>
                            </div>
                        </div>
                    </div>
                    <div class="simple-line-separator my4"></div>
                    <div style="margin-top: 14px">
                        <div>
                            <span class="block fs12 gray bold">Role to be granted :</span>
                            <h3 class="no-margin forum-color fs20">{{ $role->role }} role</h3>
                        </div>
                    </div>
                    <div style="margin-top: 14px">
                        <div>
                            <span class="block fs12 gray bold mb4">Role permissions :</span>
                            <div class="flex flex-wrap section-style y-auto-overflow" style="padding: 10px; max-height: 200px;">
                                @foreach($role_scoped_permissions as $scope=>$permissions)
                                    <span class="block bold blue fs11 mb4" style="flex-basis: 100%">{{ ucfirst($scope) }}</span>
                                    @foreach($permissions as $permission)
                                    <div class="button-style-4 mr4 mb4 fs11">{{ $permission->permission }}</div>
                                    @endforeach
                                @endforeach
                                @if(!$role_scoped_permissions->count())
                                <div class="flex align-center">
                        ...
                        <div class="relative">
                            ...
                            <div class="relative">
                                <div class="relative">
                                    ...
                                    <input type="text" class="search-input-style full-width border-box" id="role-member-search-input" autocomplete="off" placeholder="find member by username">
                                    <div class="button-style-4 full-center absolute" id="role-search-for-member-to-grant" style="right: 0; top: 0; height: 18px;">
                            ...
                            <div class="relative">
                                <div id="role-members-search-result-box" class="full-width scrolly none" style="max-height: 246px;">
                                    ...
                                    <div class="role-member-search-user role-member-search-user-factory mb4 flex none">
                                        ...
                                        <div class="button-style-4 flex align-center ml8 height-max-content role-select-member none" style="padding: 6px 10px">
                                        ...
                                        <div class="flex align-center ml8 height-max-content already-has-role none" style="padding: 2px">
                    ...
                    <div style="margin-top: 12px">
                        ...
                        <div id="empty-role-members-selected-box" class="flex align-center section-style">
                            <svg class="size14 mr8" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path d="M256,0C114.5,0,0,114.51,0,256S114.51,512,256,512,512,397.49,512,256,397.49,0,256,0Zm0,472A216,216,0,1,1,472,256,215.88,215.88,0,0,1,256,472Zm0-257.67a20,20,0,0,0-20,20V363.12a20,20,0,0,0,40,0V234.33A20,20,0,0,0,256,214.33Zm0-78.49a27,27,0,1,1-27,27A27,27,0,0,1,256,135.84Z"/></svg>
                        ...
                        <div class="selected-role-member-to-get-role selected-role-member-to-get-role-factory mb4 mr4 full-center flex-column relative none">
                            ...
                            <a href="" class="sum-profilelink" style="height: 54px">
                                <img src="{{ auth()->user()->sizedavatar(100, '-h') }}" class="size48 rounded mr8 sum-avatar" alt="" style="border: 3px solid #9f9f9f;">
                            </a>
                    ...
                    <div style="margin-top: 12px">
                        ...
                        <div>
                            <input type="text" autocomplete="off" class="full-width input-style-1" id="grant-role-confirm-input" style="padding: 8px 10px" placeholder="role grant confirmation">
                            <input type="hidden" id="grant-role-confirm-value" autocomplete="off" value="{{ auth()->user()->username }}::grant-role::{{ $role->slug }}">
                        </div>
                        <div class="flex" style="margin-top: 12px">
                            <div class="flex align-center full-width">
                                <div id="grant-role-button" class="disabled-green-button-style green-button-style full-center full-width">
                                    <div class="relative size14 mr4">
            ...
            <div class="viewer-box-style-1" style="width: 600px;">
                <div class="flex align-center space-between light-gray-border-bottom" style="padding: 14px;">
                    <div class="flex align-center">
                        ...
                        <span class="fs20 bold forum-color">{{ __('Revoke role from users') }}</span>
                    </div>
                    <div class="pointer fs20 close-global-viewer unselectable">✖</div>
                </div>
        ...
        @if($can_attach_permissions_to_role)
            ...
            <div class="viewer-box-style-1" style="width: 600px;">
                <div class="flex align-center space-between light-gray-border-bottom" style="padding: 14px;">
                    <div class="flex align-center">
                        ...
                        <span class="fs20 bold forum-color">Attach permissions to role</span>
                    </div>
                    <div class="pointer fs20 close-global-viewer unselectable">✖</div>
                </div>
                <div class="viewer-scrollable-box scrolly" style="padding: 14px; max-height: 430px">
                    ...
                    <div class="section-style fs13 lblack mb8">
                        <span class="fs15 bold forum-color">Attach permissions to "<span class="blue">{{ $role->role }}</span>" role</span>
                        <p class="no-margin mt8">Here you can attach permissions to "{{ $role->role }}" role. Before proceding this process, consider the following points</p>
                        <div class="ml8 mt8">
                            <div class="flex mt4">
                                <div class="fs10 mr8 mt4 gray">•</div>
                    ...
                    <div style="margin-top: 14px">
                        <div>
                            <span class="block fs12 forum-color bold">Role :</span>
                            <h3 class="no-margin forum-color fs20">{{ $role->role }} role</h3>
                        </div>
                    </div>
                    <div style="margin-top: 14px">
                        <div>
                            ...
                            <div id="all-permissions-box" class="flex flex-wrap section-style y-auto-overflow" style="padding: 10px; max-height: 160px;">
                                @foreach($all_permissions_scoped as $scope=>$permissions)
                                    <span class="block bold blue fs11 mb4" style="flex-basis: 100%">{{ ucfirst($scope) }}</span>
                                    @foreach($permissions as $permission)
                                        @if($permission->already_attached_to_role($role->slug))
                                        <div class="already-attached-permission-button-style mr4 mb4 fs11" title="Permission already attached to '{{ $role->role }}' role">
                                            <span>{{ $permission->permission }}</span>
                                            <span class="block fs10 normal-weight">(already atached)</span>
                                        </div>
                                        @else
                                        <div class="button-style-4 flex align-center mr4 mb4 fs11 height-max-content select-permission-to-attach-to-role">
                    ...
                    <div style="margin-top: 12px">
                        <span class="block mb4 fs12 forum-color bold">Selected permissions</span>
                        <div id="empty-role-permissions-selected-box" class="flex align-center section-style">
                            <svg class="size14 mr8" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path d="M256,0C114.5,0,0,114.51,0,256S114.51,512,256,512,512,397.49,512,256,397.49,0,256,0Zm0,472A216,216,0,1,1,472,256,215.88,215.88,0,0,1,256,472Zm0-257.67a20,20,0,0,0-20,20V363.12a20,20,0,0,0,40,0V234.33A20,20,0,0,0,256,214.33Zm0-78.49a27,27,0,1,1-27,27A27,27,0,0,1,256,135.84Z"/></svg>
                    ...
                    <div style="margin-top: 12px">
                        <p class="no-margin mb2 bold forum-color">Confirmation</p>
                        <p class="no-margin mb4 bblack">Please type <strong>{{ auth()->user()->username }}::attach-permissions-to-role::{{ $role->slug }}</strong> to confirm.</p>
                        <div>
                            <input type="text" autocomplete="off" class="full-width input-style-1" id="attach-permissions-to-role-confirm-input" style="padding: 8px 10px" placeholder="attach permission(s) from role confirmation">
                        ...
                        <div class="flex align-center" style="margin-top: 12px">
                            <div id="attach-permissions-to-role-button" class="disabled-green-button-style green-button-style full-center">
                                <div class="relative size14 mr4">
        ...
        @if($can_detach_permissions_from_role)
            ...
            <div class="viewer-box-style-1" style="width: 600px;">
                <div class="flex align-center space-between light-gray-border-bottom" style="padding: 14px;">
                    <div class="flex align-center">
                        ...
                        <span class="fs20 bold forum-color">Detach permissions from role</span>
                    </div>
                    <div class="pointer fs20 close-global-viewer unselectable">✖</div>
                </div>
                <div class="viewer-scrollable-box scrolly" style="padding: 14px; max-height: 430px">
                    ...
                    <div class="section-style fs13 lblack mb8">
                        <span class="fs15 bold forum-color">Detach permissions from "<span class="blue">{{ $role->role }}</span>" role</span>
                        <p class="no-margin mt8">Here you can detach permissions from "{{ $role->role }}" role. Before proceding this process, consider the following points</p>
                        <div class="ml8 mt8">
                            <div class="flex mt4">
                                <div class="fs10 mr8 mt4 gray">•</div>
                    ...
                    <div style="margin-top: 14px">
                        <div>
                            ...
                            <span class="block fs12 gray my4 lh15">The following list of permissions are already attached to "<strong>{{ $role->role }}</strong>" role. Choose only permission(s) that you want to detach.</span>
                            <div id="all-permissions-box" class="section-style y-auto-overflow" style="padding: 10px; max-height: 250px;">
                                @foreach($role_scoped_permissions as $scope => $permissions)
                                    <span class="block bold blue fs11 mb4" style="flex-basis: 100%">{{ ucfirst($scope) }}</span>
                                    <div class="flex flex-wrap mb8">
                                        @foreach($permissions as $permission)
                                        <div class="button-style-4 flex align-center mr4 mb4 fs11 height-max-content select-permission-to-detach-from-role">
                                            <span class="permission-name">{{ $permission->permission }}</span>
                                            <svg class="size8 ml4 x-ico" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 260 260"><path d="M156.22,3.31c3.07,2.55,4.08,5.71,4.06,9.78-.17,27.07,0,54.14-.18,81.21,0,3.57.69,4.66,4.49,4.63,27.24-.19,54.47-.11,81.71-.1,7.36,0,9.39,2,9.4,9.25q0,21.4,0,42.82c0,7-2.1,9.06-9.09,9.06-27.24,0-54.48.09-81.71-.09-3.85,0-4.83.95-4.8,4.81.17,27.07.1,54.14.09,81.21,0,7.65-1.94,9.59-9.56,9.6q-21.4,0-42.82,0c-6.62,0-8.75-2.19-8.75-8.91,0-27.4-.1-54.8.09-82.2,0-3.8-1.06-4.51-4.62-4.49-27.08.16-54.15,0-81.22.18-4.07,0-7.23-1-9.78-4.06V102.8c2.55-3.08,5.72-4.08,9.79-4.06,27.09.17,54.18,0,81.27.18,3.68,0,4.58-.87,4.55-4.56-.17-27.09,0-54.18-.18-81.27,0-4.06,1-7.23,4.06-9.78Z"></path></svg>
                                            <svg class="size10 ml4 v-ico none" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path d="M433.73,49.92,178.23,305.37,78.91,206.08.82,284.17,178.23,461.56,511.82,128Z"></path></svg>
                                            <input type="hidden" class="pid" value="{{ $permission->id }}" autocomplete="off">
                    ...
                    <div style="margin-top: 12px">
                        <span class="block mb4 fs12 forum-color bold">Selected permissions</span>
                        <div id="empty-role-permissions-to-detach-selected-box" class="flex align-center section-style">
                            <svg class="size14 mr8" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path d="M256,0C114.5,0,0,114.51,0,256S114.51,512,256,512,512,397.49,512,256,397.49,0,256,0Zm0,472A216,216,0,1,1,472,256,215.88,215.88,0,0,1,256,472Zm0-257.67a20,20,0,0,0-20,20V363.12a20,20,0,0,0,40,0V234.33A20,20,0,0,0,256,214.33Zm0-78.49a27,27,0,1,1-27,27A27,27,0,0,1,256,135.84Z"/></svg>
                    ...
                    <div style="margin-top: 12px">
                        <p class="no-margin mb2 bold forum-color">Confirmation</p>
                        <p class="no-margin mb4 bblack">Please type <strong>{{ auth()->user()->username }}::detach-permissions-from-role::{{ $role->slug }}</strong> to confirm.</p>
                        <div>
                            <input type="text" autocomplete="off" class="full-width input-style-1" id="detach-permissions-from-role-confirm-input" style="padding: 8px 10px" placeholder="detach permission(s) from role confirmation">
                        ...
                        <div class="flex align-center" style="margin-top: 12px">
                            <div id="detach-permissions-from-role-button" class="disabled-red-button-style red-button-style full-center">
                                <div class="relative size14 mr4">
            ...
            <div class="viewer-box-style-1" style="width: 600px;">
                <div class="flex align-center space-between light-gray-border-bottom" style="padding: 14px;">
                    <div class="flex align-center">
                        ...
                        <span class="fs20 bold forum-color">Delete role</span>
                    </div>
                    <div class="pointer fs20 close-global-viewer unselectable">✖</div>
                </div>
                <div class="viewer-scrollable-box scrolly" style="padding: 14px; max-height: 430px">
                    <input type="hidden" id="rum-at-least-one-selected-message" value="You need to select at least one member to attach role into" autocomplete="off">
                    <div class="section-style fs13 lblack mb8">
                        <span class="block fs18 bold lblack mb8">Delete "<span class="blue">{{ $role->role }}</span>" role</span>
                        <p class="no-margin">Here you can delete "{{ $role->role }}" role thoroughly. Before proceding this process, consider the following points</p>
                        <div class="ml8 mt8">
                            <div class="flex mt4">
                                <div class="fs10 mr8 mt4 gray">•</div>
                                <p class="no-margin" style="line-height: 1.5">If the role is deleted, <strong>all its associated permissions will be detached from every member with that role</strong>. To understand the way permissions and roles relate, consider the following notes.</p>
                            </div>
                            <div class="flex mt4">
                                <div class="fs10 mr8 mt4 gray">•</div>
                                <p class="no-margin" style="line-height: 1.5">All members with that role got permissions associated to this role; And so deleting role will delete those permissions from those members. If a user has this role and also <strong>has a higher role</strong>, the permissions will not be detached in this case from this user.</p>
                            </div>
                            <div class="flex mt4">
                                <div class="fs10 mr8 mt4 gray">•</div>
                                <p class="no-margin" style="line-height: 1.5">When the role get deleted, certain permissions may become orphaned; In this case a site owner must organize the permissions by either attach them to other role or create new role with better permissions order.</p>
                            </div>
                        </div>
                    </div>
                    <div class="simple-line-separator my4"></div>
                    <div style="margin-top: 14px">
                        <div>
                            <span class="block fs12 gray bold">Role to be <span class="red">deleted</span> :</span>
                            <h3 class="no-margin forum-color fs20">{{ $role->role }} role</h3>
                        </div>
                    </div>
                    <div style="margin-top: 14px">
                    ...
                    <div style="margin-top: 14px">
                        ...
                        <div class="flex flex-wrap section-style border-box" style="padding: 10px; max-height: 250px; overflow-y: auto;">
                            ...
                            <div class="flex align-center flex-column mx8 my8">
                                <img src="{{ $user->sizedavatar(100, '-h') }}" class="rounded-entity-for-role" alt="">
                                <span class="bold blue fs11 mt4">{{ $user->username }}</span>
                                <span class="bold lblack fs10">{{ $user->high_role()->role }}</span>
                            </div>
                            @endforeach
                            @if(!$users->count())
                            <div class="flex align-center">
                    ...
                    <div style="margin-top: 12px">
                        <p class="no-margin mb2 bold forum-color">Confirmation</p>
                        <p class="no-margin mb4 bblack">Please type <strong>{{ auth()->user()->username }}::delete-role::{{ $role->slug }}</strong> to confirm.</p>
                        <div>
                            <input type="text" autocomplete="off" class="full-width input-style-1" id="delete-role-confirm-input" style="padding: 8px 10px" placeholder="role delete confirmation">
                            <input type="hidden" id="delete-role-confirm-value" autocomplete="off" value="{{ auth()->user()->username }}::delete-role::{{ $role->slug }}">
                        </div>
                        <div class="flex" style="margin-top: 12px">
                            <div class="flex align-center">
                                <div id="delete-role-button" class="disabled-red-button-style red-button-style flex align-center">
                                    <div class="relative size14 mr4">
        ...
        <div class="flex align-center space-between" style="margin-bottom: 12px">
            <div class="flex align-center">
                ...
                <h2 class="no-margin fs20 mb2 lblack">Manage "<span class="blue">{{$role->role }}</span>" role</h2>
            </div>
            @if($canadd)
            <div class="green-button-style width-max-content flex align-center my4 open-create-role-dialog">
                <svg class="size10 mr8" fill="white" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 260 260"><path d="M156.22,3.31c3.07,2.55,4.08,5.71,4.06,9.78-.17,27.07,0,54.14-.18,81.21,0,3.57.69,4.66,4.49,4.63,27.24-.19,54.47-.11,81.71-.1,7.36,0,9.39,2,9.4,9.25q0,21.4,0,42.82c0,7-2.1,9.06-9.09,9.06-27.24,0-54.48.09-81.71-.09-3.85,0-4.83.95-4.8,4.81.17,27.07.1,54.14.09,81.21,0,7.65-1.94,9.59-9.56,9.6q-21.4,0-42.82,0c-6.62,0-8.75-2.19-8.75-8.91,0-27.4-.1-54.8.09-82.2,0-3.8-1.06-4.51-4.62-4.49-27.08.16-54.15,0-81.22.18-4.07,0-7.23-1-9.78-4.06V102.8c2.55-3.08,5.72-4.08,9.79-4.06,27.09.17,54.18,0,81.27.18,3.68,0,4.58-.87,4.55-4.56-.17-27.09,0-54.18-.18-81.27,0-4.06,1-7.23,4.06-9.78Z"></path></svg>
            ...
            @else
            <div class="disabled-green-button-style green-button-style width-max-content flex align-center my4" title="you don't have permission to create roles">
                <svg class="size10 mr8" fill="white" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 260 260"><path d="M156.22,3.31c3.07,2.55,4.08,5.71,4.06,9.78-.17,27.07,0,54.14-.18,81.21,0,3.57.69,4.66,4.49,4.63,27.24-.19,54.47-.11,81.71-.1,7.36,0,9.39,2,9.4,9.25q0,21.4,0,42.82c0,7-2.1,9.06-9.09,9.06-27.24,0-54.48.09-81.71-.09-3.85,0-4.83.95-4.8,4.81.17,27.07.1,54.14.09,81.21,0,7.65-1.94,9.59-9.56,9.6q-21.4,0-42.82,0c-6.62,0-8.75-2.19-8.75-8.91,0-27.4-.1-54.8.09-82.2,0-3.8-1.06-4.51-4.62-4.49-27.08.16-54.15,0-81.22.18-4.07,0-7.23-1-9.78-4.06V102.8c2.55-3.08,5.72-4.08,9.79-4.06,27.09.17,54.18,0,81.27.18,3.68,0,4.58-.87,4.55-4.56-.17-27.09,0-54.18-.18-81.27,0-4.06,1-7.23,4.06-9.78Z"></path></svg>
                <span class="fs11 bold">create new role</span>
            </div>
            @endif
        ...
        <div class="flex" style="margin-bottom: 16px">
            <div class="role-section half-width mr8">
                <!-- update/edit role informations -->
                <div class="flex align-center mb8">
                    ...
                    <p class="no-margin bold forum-color fs16">Role Informations</p>
                </div>
                <div class="section-style mb8">
                    <span class="block bold lblack mb8">Important</span>
                    <p class="no-margin fs13 mb4 lblack lh15">Please be careful when you're updating because some roles attributes (like slug) are used in codebase checks to verify user authorization for performing activities.</p>
                    <p class="no-margin fs13 lblack lh15">Don't update role informations unless you are 100% sure about it.(we provided a confirmation before updating)</p>
                </div>
                ...
                <div style="max-width: 600px">
                    <div class="mb8">
                        <label for="update-role-name-input" class="flex align-center bold forum-color fs13">{{ __('Name') }}<span class="ml4 err red none fs12">*</span></label>
                        <p class="no-margin fs12 mb2 gray">Role name should not contain commas(,)</p>
                        <input type="text" autocomplete="off" class="input-style-1 full-width" id="update-role-name-input" placeholder="Role name" value="{{ $role->role }}" style="padding: 8px 10px">
                    </div>
                    <div class="mb8">
                        <label for="update-role-slug-input" class="flex align-center bold forum-color fs13">{{ __('Slug') }}<span class="ml4 err red none fs12">*</span></label>
                        <p class="no-margin fs12 mb2 gray">Role slug also should not contain commas(,)</p>
                        <input type="text" autocomplete="off" class="input-style-1 full-width" id="update-role-slug-input" placeholder="Role slug" value="{{ $role->slug }}" style="padding: 8px 10px">
                    </div>
                    <div class="mb8">
                        <label for="update-role-description-input" class="flex align-center bold forum-color mb4 fs13">{{ __('Description') }}<span class="ml4 err red none fs12">*</span></label>
                        <textarea id="update-role-description-input" class="styled-textarea fs13"
                            style="margin: 0; padding: 8px; width: 100%; min-height: 110px; max-height: 110px;"
                            maxlength="800"
                            spellcheck="false"
                            autocomplete="off"
                    ...
                    <div style="margin-bottom: 14px">
                        <label class="flex align-center bold forum-color fs13">{{ __('Priority') }}<span class="ml4 err red none fs12">*</span></label>
                        <p class="no-margin fs12 mb2 gray">Updating role priority require update other roles priorities. To change roles priorities go to R&P management page and select manage roles priorities</p>
                    </div>
                    <label for="update-role-confirm-input" class="block mb4 bold forum-color">Confirmation</label>
                    <p class="no-margin mb4 lblack">Please type <strong>{{ auth()->user()->username }}::edit-role::{{ $role->slug }}</strong> to confirm.</p>
                    <div>
                        <input type="text" autocomplete="off" class="full-width input-style-1" id="update-role-confirm-input" style="padding: 8px 10px" placeholder="confirmation">
                        <input type="hidden" id="update-role-confirm-value" autocomplete="off" value="{{ auth()->user()->username }}::edit-role::{{ $role->slug }}">
                    </div>
                    @if(!$canupdate)
                    <div class="section-style flex align-center my8">
                        ...
                        <p class="fs12 bold lblack no-margin">You don't have permission to update roles informations</p>
                    </div>
                    @endif
                    <div class="flex" style="margin-top: 12px">
                        @if($canupdate)
                        <div id="update-role-button" class="disabled-green-button-style green-button-style full-center">
                            <div class="relative size14 mr4">
                                <svg class="size12 icon-above-spinner" fill="white" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path d="M458.18,217.58a23.4,23.4,0,0,0-23.4,23.4V428.2a23.42,23.42,0,0,1-23.4,23.4H83.76a23.42,23.42,0,0,1-23.4-23.4V100.57a23.42,23.42,0,0,1,23.4-23.4H271a23.4,23.4,0,0,0,0-46.8H83.76a70.29,70.29,0,0,0-70.21,70.2V428.2a70.29,70.29,0,0,0,70.21,70.2H411.38a70.29,70.29,0,0,0,70.21-70.2V241A23.39,23.39,0,0,0,458.18,217.58Zm-302,56.25a11.86,11.86,0,0,0-3.21,6l-16.54,82.75a11.69,11.69,0,0,0,11.49,14,11.26,11.26,0,0,0,2.29-.23L233,359.76a11.68,11.68,0,0,0,6-3.21L424.12,171.4,341.4,88.68ZM481.31,31.46a58.53,58.53,0,0,0-82.72,0L366.2,63.85l82.73,82.72,32.38-32.39a58.47,58.47,0,0,0,0-82.72ZM155.72,273.08a11.86,11.86,0,0,0-3.21,6L136,361.8a11.69,11.69,0,0,0,11.49,14,11.26,11.26,0,0,0,2.29-.23L232.48,359a11.68,11.68,0,0,0,6-3.21L423.62,170.65,340.9,87.93ZM480.81,30.71a58.53,58.53,0,0,0-82.72,0L365.7,63.1l82.73,82.72,32.38-32.39a58.47,58.47,0,0,0,0-82.72Z"/></svg>
                                <svg class="spinner size14 opacity0 absolute" style="top: 0; left: 0" fill="none" viewBox="0 0 16 16">
                                    <circle cx="8" cy="8" r="7" stroke="currentColor" stroke-opacity="0.25" stroke-width="2" vector-effect="non-scaling-stroke"></circle>
                            ...
                            <span class="bold">Update role informations</span>
                        </div>
                        @else
                        <div class="disabled-green-button-style green-button-style full-center">
                            <svg class="size12 mr4" fill="white" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path d="M458.18,217.58a23.4,23.4,0,0,0-23.4,23.4V428.2a23.42,23.42,0,0,1-23.4,23.4H83.76a23.42,23.42,0,0,1-23.4-23.4V100.57a23.42,23.42,0,0,1,23.4-23.4H271a23.4,23.4,0,0,0,0-46.8H83.76a70.29,70.29,0,0,0-70.21,70.2V428.2a70.29,70.29,0,0,0,70.21,70.2H411.38a70.29,70.29,0,0,0,70.21-70.2V241A23.39,23.39,0,0,0,458.18,217.58Zm-302,56.25a11.86,11.86,0,0,0-3.21,6l-16.54,82.75a11.69,11.69,0,0,0,11.49,14,11.26,11.26,0,0,0,2.29-.23L233,359.76a11.68,11.68,0,0,0,6-3.21L424.12,171.4,341.4,88.68ZM481.31,31.46a58.53,58.53,0,0,0-82.72,0L366.2,63.85l82.73,82.72,32.38-32.39a58.47,58.47,0,0,0,0-82.72ZM155.72,273.08a11.86,11.86,0,0,0-3.21,6L136,361.8a11.69,11.69,0,0,0,11.49,14,11.26,11.26,0,0,0,2.29-.23L232.48,359a11.68,11.68,0,0,0,6-3.21L423.62,170.65,340.9,87.93ZM480.81,30.71a58.53,58.53,0,0,0-82.72,0L365.7,63.1l82.73,82.72,32.38-32.39a58.47,58.47,0,0,0,0-82.72Z"/></svg>
                            <span class="bold">Update role informations</span>
                        </div>
                        @endif
                    </div>
            ...
            <div class="role-section half-width ml8">
                <!-- manage members with that role -->
                <div class="flex align-center mb4">
                    ...
                    <p class="no-margin bold forum-color fs16">Members with that role</p>
                </div>
                <p class="my4 fs12 lblack lh15">The following members have this role already. You can attach this role to more members as well as revoking it from its owners.</p>

                <div class="flex justify-center flex-wrap section-style border-box" style="padding: 10px; margin: 12px 0; max-height: 250px; overflow-y: auto;">
                    <style>
                    ...
                    @else
                    <div class="mx8 my8 full-center cursor-not-allowed rounded-entity-for-role" title="You don't have permission to grant roles to users" style="border-color: #959595;">
                        <svg class="size10" fill="#737373" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 260 260"><path d="M156.22,3.31c3.07,2.55,4.08,5.71,4.06,9.78-.17,27.07,0,54.14-.18,81.21,0,3.57.69,4.66,4.49,4.63,27.24-.19,54.47-.11,81.71-.1,7.36,0,9.39,2,9.4,9.25q0,21.4,0,42.82c0,7-2.1,9.06-9.09,9.06-27.24,0-54.48.09-81.71-.09-3.85,0-4.83.95-4.8,4.81.17,27.07.1,54.14.09,81.21,0,7.65-1.94,9.59-9.56,9.6q-21.4,0-42.82,0c-6.62,0-8.75-2.19-8.75-8.91,0-27.4-.1-54.8.09-82.2,0-3.8-1.06-4.51-4.62-4.49-27.08.16-54.15,0-81.22.18-4.07,0-7.23-1-9.78-4.06V102.8c2.55-3.08,5.72-4.08,9.79-4.06,27.09.17,54.18,0,81.27.18,3.68,0,4.58-.87,4.55-4.56-.17-27.09,0-54.18-.18-81.27,0-4.06,1-7.23,4.06-9.78Z"></path></svg>
                    </div>
                    @endif
                    ...
                    <div class="flex align-center flex-column mx8 my8">
                        <div class="relative">
                            <img src="{{ $user->sizedavatar(100, '-h') }}" class="rounded-entity-for-role" alt="">
                            @if($can_revoke_role_from_user)
                            <div class="open-revoke-role-dialog x-close-container-style" style="right: -13px; top: -8px;">
                                <span class="x-close unselectable">✖</span>
                ...
                <div class="flex align-center mb4">
                    ...
                    <p class="no-margin bold forum-color fs16">Role permissions</p>
                </div>
                <p class="my4 fs12 lblack lh15">The following permissions are attached with that "{{ $role->role }}"; It means If a member get this role, It will automatically get all those permissions.</p>
                <div class="block mt8 gray height-max-content fs10 bold">• Attach/Detach permissions</div>
                <div class="flex align-center">
                    @if($can_attach_permissions_to_role)
                    <div class="green-button-style width-max-content flex align-center my4 open-attach-permissions-to-role-dialog">
                    ...
                    @else
                    <div class="green-button-style disabled-green-button-style width-max-content flex align-center my4" title="You don't have permission to attach permissions to roles">
                        <svg class="size10 mr4" fill="white" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 260 260"><path d="M156.22,3.31c3.07,2.55,4.08,5.71,4.06,9.78-.17,27.07,0,54.14-.18,81.21,0,3.57.69,4.66,4.49,4.63,27.24-.19,54.47-.11,81.71-.1,7.36,0,9.39,2,9.4,9.25q0,21.4,0,42.82c0,7-2.1,9.06-9.09,9.06-27.24,0-54.48.09-81.71-.09-3.85,0-4.83.95-4.8,4.81.17,27.07.1,54.14.09,81.21,0,7.65-1.94,9.59-9.56,9.6q-21.4,0-42.82,0c-6.62,0-8.75-2.19-8.75-8.91,0-27.4-.1-54.8.09-82.2,0-3.8-1.06-4.51-4.62-4.49-27.08.16-54.15,0-81.22.18-4.07,0-7.23-1-9.78-4.06V102.8c2.55-3.08,5.72-4.08,9.79-4.06,27.09.17,54.18,0,81.27.18,3.68,0,4.58-.87,4.55-4.56-.17-27.09,0-54.18-.18-81.27,0-4.06,1-7.23,4.06-9.78Z"></path></svg>
                        <span class="fs11 bold">attach new permissions</span>
                    </div>
                    @endif
                    ...
                    @if($can_detach_permissions_from_role)
                    <div class="red-button-style width-max-content flex align-center my4 open-detach-permissions-from-role-dialog">
                    ...
                    @else
                    <div class="red-button-style disabled-red-button-style width-max-content flex align-center my4" title="You don't have permission to detach permissions from roles">
                        <svg class="size12 mr4" fill="white" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 260 260"><path d="M2.19,144V114.32c2.06-1.67,1.35-4.2,1.78-6.3Q19.81,30.91,94.83,7.28c6.61-2.07,13.5-3.26,20.26-4.86h26.73c1.44,1.93,3.6.92,5.39,1.2C215,14.2,261.83,74.5,254.91,142.49c-6.25,61.48-57.27,110-119,113.3A127.13,127.13,0,0,1,4.9,155.18C4.09,151.45,4.42,147.42,2.19,144Zm126.75-30.7c-19.8,0-39.6.08-59.4-.08-3.24,0-4.14.82-4.05,4,.24,8.08.21,16.17,0,24.25-.07,2.83.77,3.53,3.55,3.53q59.89-.14,119.8,0c2.8,0,3.6-.74,3.53-3.54-.18-8.08-.23-16.17,0-24.25.1-3.27-.85-4.06-4.06-4C168.55,113.4,148.75,113.33,128.94,113.33Z"/></svg>
                        <span class="fs11 bold">detach permissions</span>
                    </div>
                    @endif
                </div>
                <div class="section-style border-box" style="padding: 10px; margin: 2px 0 12px 0; max-height: 250px; overflow-y: auto">
                    <div class="block gray height-max-content fs10 bold">• permissions attached</div>
                ...
                <div class="flex align-center mb4">
                    ...
                    <p class="no-margin bold forum-color fs16">Delete role</p>
                </div>
                <p class="my4 fs13 lblack lh15">Deleting the role will revoke it from all members who have this role already.</p>
                ...
                @if($can_delete_role)
                <div class="red-button-style width-max-content flex align-center mt8 open-delete-role-dialog">
                    ...
                    <span class="fs11 bold">Delete role</span>
                </div>
                @else
                <div class="red-button-style disabled-red-button-style width-max-content flex align-center mt8" title="You don't have permission to delete roles">
                    <svg class="size12 mr4" fill="white" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path d="M300,416h24a12,12,0,0,0,12-12V188a12,12,0,0,0-12-12H300a12,12,0,0,0-12,12V404A12,12,0,0,0,300,416ZM464,80H381.59l-34-56.7A48,48,0,0,0,306.41,0H205.59a48,48,0,0,0-41.16,23.3l-34,56.7H48A16,16,0,0,0,32,96v16a16,16,0,0,0,16,16H64V464a48,48,0,0,0,48,48H400a48,48,0,0,0,48-48h0V128h16a16,16,0,0,0,16-16V96A16,16,0,0,0,464,80ZM203.84,50.91A6,6,0,0,1,209,48h94a6,6,0,0,1,5.15,2.91L325.61,80H186.39ZM400,464H112V128H400ZM188,416h24a12,12,0,0,0,12-12V188a12,12,0,0,0-12-12H188a12,12,0,0,0-12,12V404A12,12,0,0,0,188,416Z"/></svg>
                    <span class="fs11 bold">Delete role</span>
                </div>
                @endif

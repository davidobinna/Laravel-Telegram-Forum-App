...
@section('title', 'Admin - Archives')

@section('left-panel')
    @include('partials.admin.left-panel', ['page'=>'admin.archives'])
@endsection
    ...
    <div id="admin-main-content-box">
        <div class="flex space-between">
            ...
            <div class="flex align-center height-max-content">
                ...
                <div class="flex align-center">
                    <span class="fs13 bold">{{ __('Archives') }}</span>
                </div>
            </div>
        </div>

        <div class="section-style fs13">
            <p class="no-margin mb8 lblack">Here, in archives page where you can handle all <strong>archived resources</strong> as well as old and deleted entities in the system.</p>
            <p class="no-margin no-margin lblack">This page allows only super admins to deal with the archives by performing actions like review, restore, and permanently delete resources.</p>
        </div>
        ...
        <div class="red-section-style my8">
            <div class="flex fs13">
                ...
                <div>
                    <p class="no-margin">You <strong>cannot access this page</strong> because you don't have permission to do so. Only super admins or above can access.</p>
                </div>
            </div>
        </div>
        @else
        <div class="flex align-center">
            ...
            <p class="no-margin lblack my8">Every section include a separate viewer to handle specific task, to make procedures clean and organized.</p>
        </div>

        <div class="toggle-box mt8">
            <div class="flex align-center pointer toggle-container-button">
                <h2 class="no-margin forum-color">Archived Forums</h2>
                <svg class="ml8 mt2 size10 toggle-arrow" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 30.02 30.02">
                    <path d="M13.4,1.43l9.35,11a4,4,0,0,1,0,5.18l-9.35,11a4,4,0,1,1-6.1-5.18L14.46,15,7.3,6.61a4,4,0,0,1,6.1-5.18Z"></path>
                </svg>
            </div>
            <p class="fs13 lblack my4">This section concerns archived forums operations like permanent delete and more.</p>
            <div class="toggle-container scrolly mt8" style="max-height: 320px; padding-right: 14px">
                <table class="full-width">
                    <tr>
                        <th id="forum-name-column">Archived forum</th>
                    ...
                    @foreach($archivedforums as $forum)
                        @php
                            ...
                            $approver = $forum->approver;
                        @endphp
                        <tr class="forum-row">
                            <input type="hidden" class="category-id" autocomplete="off" value="">
                            <td class="bold">{{ $forum->forum }}</td>
                            <td>
                                <div class="expand-box">
                                    <span class="expandable-text fs13 my4 forum-description" style="line-height: 1.4">{{ $forum->description_slice }}</span>
                                    @if($forum->description_slice != $forum->description)
                                    <input type="hidden" class="expand-slice-text" value="{{ $forum->description_slice }}" autocomplete="off">
                                ...
                                <div class="flex align-center my4">
                                    <p class="no-margin fs13 lblack">* <strong>created</strong> by : <a href="{{ route('user.profile', ['user'=>$creator->username]) }}" target="_blank" class="bold blue no-underline ">{{ $creator->username }}</a></p>
                                ...
                                <div class="my8 fs13">
                                    <div class="ml8">
                                        <div class="flex align-center">
                                            ...
                                            <p class="my8 bold">Total threads : {{ $forum->statistics('total-threads-count') }}</p>
                                        </div>
                                        <div class="fs10 mx8 gray" style="align-self: center;">•</div>
                                        <div class="flex align-center">
                                            ...
                                            <p class="my8 bold">Total replies : {{ $forum->statistics('total-posts-count') }}</p>
                                        </div>
                                        <div class="fs10 mx8 gray" style="align-self: center;">•</div>
                                        <div>
                                            <div class="flex align-center">
                                                <div class="flex align-center">
                                                    ...
                                                    <p class="my8 bold">Total votes on :</p>
                                                </div>
                                                <div class="ml8">
                                                    <p class="no-margin">Threads : <span class="bold">{{ $forum->statistics('total-threads-votes-count') }}</span> </p>
                                                    <p class="no-margin">Replies : <span class="bold">{{ $forum->statistics('total-posts-votes-count') }}</span> </p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <td>
                                <a href="{{ route('admin.forum.restore') . '?forumid=' . $forum->id }}" class="button-style-2 full-center no-underline black mt4">
                                ...
                                <a href="#" class="red-button-style full-center no-underline black mt4" style="padding: 5px 12px;">
                    ...
                    @endforeach
                    @if(!$archivedforums->count())
                    <tr>
                        <td colspan="4">
                            <div class="full-center">
                                <svg class="size14 mr4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path d="M256,0C114.5,0,0,114.51,0,256S114.51,512,256,512,512,397.49,512,256,397.49,0,256,0Zm0,472A216,216,0,1,1,472,256,215.88,215.88,0,0,1,256,472Zm0-257.67a20,20,0,0,0-20,20V363.12a20,20,0,0,0,40,0V234.33A20,20,0,0,0,256,214.33Zm0-78.49a27,27,0,1,1-27,27A27,27,0,0,1,256,135.84Z"></path></svg>
                                <p class="my4 text-center">No forums are archived at the moment.</p>
                            </div>
                        </td>
                    </tr>
                    @endif
        ...
        <div class="toggle-box" style="margin-top: 12px">
            <div class="flex align-center pointer toggle-container-button">
                <h2 class="no-margin forum-color">Archived Categories</h2>
                <svg class="ml8 mt2 size10 toggle-arrow" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 30.02 30.02">
                    <path d="M13.4,1.43l9.35,11a4,4,0,0,1,0,5.18l-9.35,11a4,4,0,1,1-6.1-5.18L14.46,15,7.3,6.61a4,4,0,0,1,6.1-5.18Z"></path>
                </svg>
            </div>
            <p class="fs13 lblack my4">This section concerns archived categories operations.</p>
            <div class="toggle-container scrolly mt8" style="max-height: 320px; padding-right: 14px">
                <table class="full-width">
                    <tr>
                        <th id="forum-name-column">Archived category</th>
                        <th id="forum-infos-column">Infos</th>
                        <th id="forum-statistics-column">statistics</th>
                        <th id="forum-ops-column">operations</th>
                    </tr>
                    @foreach($archivedcategories as $category)
                        ...
                        <tr class="forum-row">
                            ...
                            <td>
                                <div class="expand-box">
                                    <span class="expandable-text fs13 my4" style="line-height: 1.4">{{ $category->description_slice }}</span>
                            ...
                            <td class="bold">
                                <div class="my8 fs13">
                                    <div class="ml8">
                                        <div class="flex align-center">
                                            <svg class="size15 mr4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 260 260"><path d="M130,17.11h97.27c11.82,0,15.64,3.73,15.64,15.34q0,75.07,0,150.16c0,11.39-3.78,15.13-15.22,15.13-2.64,0-5.3.12-7.93-.06a11.11,11.11,0,0,1-10.53-9.38c-.81-5.69,2-11,7.45-12.38,3.28-.84,3.52-2.36,3.51-5.06-.07-27.15-.11-54.29,0-81.43,0-3.68-1-4.69-4.68-4.68q-85.63.16-171.29,0c-3.32,0-4.52.68-4.5,4.33q.26,41,0,81.95c0,3.72,1.3,4.53,4.56,4.25a45.59,45.59,0,0,1,7.39.06,11.06,11.06,0,0,1,10.58,11c0,5.62-4.18,10.89-9.91,11.17-8.43.4-16.92.36-25.36,0-5.16-.23-8.82-4.31-9.68-9.66a33,33,0,0,1-.24-5.27q0-75.08,0-150.16c0-11.61,3.81-15.34,15.63-15.34Zm22.49,45.22c16.56,0,33.13,0,49.7,0,5.79,0,13.59,2,16.83-.89,3.67-3.31.59-11.25,1.19-17.13.4-3.92-1.21-4.54-4.73-4.51-19.21.17-38.42.08-57.63.08-22.73,0-45.47.11-68.21-.1-4,0-5.27,1-4.92,5a75.62,75.62,0,0,1,0,12.68c-.32,3.89.78,5,4.85,5C110.54,62.21,131.51,62.33,152.49,62.33ZM62.3,51.13c0-11.26,0-11.26-11.45-11.26h-.53c-10.47,0-10.47,0-10.47,10.71,0,11.75,0,11.75,11.49,11.75C62.3,62.33,62.3,62.33,62.3,51.13ZM102,118.66c25.79.3,18.21-2.79,36.49,15.23,18.05,17.8,35.89,35.83,53.8,53.79,7.34,7.35,7.3,12.82-.13,20.26q-14.94,15-29.91,29.87c-6.86,6.81-12.62,6.78-19.5-.09-21.3-21.28-42.53-42.64-63.92-63.84a16.11,16.11,0,0,1-5.24-12.62c.23-9.86,0-19.73.09-29.59.07-8.71,4.24-12.85,13-13C91.81,118.59,96.92,118.66,102,118.66ZM96.16,151c.74,2.85-1.53,6.66,1.41,9.6,17.66,17.71,35.39,35.36,53,53.11,1.69,1.69,2.59,1.48,4.12-.12,4.12-4.34,8.24-8.72,12.73-12.67,2.95-2.59,2.36-4-.16-6.49-15.68-15.46-31.4-30.89-46.63-46.79-4.56-4.76-9.1-6.73-15.59-6.35C96.18,141.8,96.16,141.41,96.16,151Z"/></svg>
                                            <p class="my8 bold">Total threads : {{ $category->statistics('total-threads-count') }}</p>
                                        </div>
                                        <div class="fs10 mx8 gray" style="align-self: center;">•</div>
                                        <div class="flex align-center">
                                            <svg class="size15 mr4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path d="M221.09,253a23,23,0,1,1-23.27,23A23.13,23.13,0,0,1,221.09,253Zm93.09,0a23,23,0,1,1-23.27,23A23.12,23.12,0,0,1,314.18,253Zm93.09,0A23,23,0,1,1,384,276,23.13,23.13,0,0,1,407.27,253Zm62.84-137.94h-51.2V42.9c0-23.62-19.38-42.76-43.29-42.76H43.29C19.38.14,0,19.28,0,42.9V302.23C0,325.85,19.38,345,43.29,345h73.07v50.58c.13,22.81,18.81,41.26,41.89,41.39H332.33l16.76,52.18a32.66,32.66,0,0,0,26.07,23H381A32.4,32.4,0,0,0,408.9,496.5L431,437h39.1c23.08-.13,41.76-18.58,41.89-41.39V156.47C511.87,133.67,493.19,115.21,470.11,115.09ZM46.55,299V46.12H372.36v69H158.25c-23.08.12-41.76,18.58-41.89,41.38V299Zm418.9,92H397.5l-15.83,46-15.82-46H162.91V161.07H465.45Z"/></svg>
                                            <p class="my8 bold">Total replies : {{ $category->statistics('total-posts-count') }}</p>
                                        </div>
                                        <div class="fs10 mx8 gray" style="align-self: center;">•</div>
                                        <div>
                                            <div class="flex align-center">

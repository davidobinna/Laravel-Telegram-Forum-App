<div>
    ...
    @if($canreview)
        @if($totalvotescount)
            <h2 class="fs14 forum-color no-margin">All user votes ({{ $totalvotescount }})</h2>
            <p class="fs13 no-margin my4">The following votes include votes on both threads and replies.</p>
            <div class="user-votes-container">
                @foreach($resources as $resource)
                    @if($resource instanceof \App\Models\Thread)
                        <x-admin.thread.thread-review-component :thread="$resource" :checkuservote="$user->id"/>
                    @elseif($resource instanceof \App\Models\Post)
                        <x-admin.post.post-review-component :post="$resource" :checkuservote="$user->id"/>
                    @endif
                @endforeach
            </div>
            @if($totalvotescount > 10)
            <div class="full-center py8" id="user-votes-review-fetch-more">
        ...
        @else
            <div class="full-center" style="margin-top: 20px;">
                ...
                <span class="bold lblack fs12">This user does not have any vote since registered</span>
            </div>
        @endif
    @else

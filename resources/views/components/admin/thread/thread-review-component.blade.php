<div class="thread-review-record section-style white-background mb8" style="padding: 8px; @if($thread->category->slug == 'announcements') background-color: #fffbe8 !important; @endif">
    <!-- thread owner -->
                ...
                <div class="flex align-center gray mt2 fs11">
                    ...
                    @php
                        ...
                        if($threadstatus->slug != 'live')
                            $scolor = 'red';
                    @endphp
                    <span class="bold fs12 {{ $scolor }} ml4">{{ $threadstatus->status }}</span>
                </div>
            </div>
        </div>
        <div class="flex align-center height-max-content">
            <div class="wtypical-button-style render-thread" style="padding: 3px 8px">
                ...
                <span class="fs11 lblack bold">preview</span>
                <input type="hidden" class="thread-id" value="{{ $thread->id }}" autocomplete="off">
            </div>
            <a href="{{ route('admin.thread.manage') . '?threadid=' . $thread->id }}" target="_blank" class="flex align-center no-underline ml8">
    ...
    <div class="mb8">
        <div class="flex">
            <span class="bold fs13 lblack mr4 no-wrap" style="margin-top: 3px">subject :</span>
            <a href="{{ $thread->link }}" class="blue no-underline bold mt2">
                <span class="fs16">{{ $thread->mediumslice }}</span>
            </a>
        </div>
    </div>

<div class="flex">
    <span class="forum-color bold mr8">Role :</span>
    <div>
        <h2 class="no-margin mr8 fs18">{{ $role->role }}</h2>
        <span class="block fs14" style="margin-top: -2px">{{ $role->slug }}</span>
    </div>
    <a href="{{ route('admin.rap.manage.role') . '?roleid=' . $role->id }}" class="button-style-4 full-center move-to-right no-underline black" style="padding: 4px 10px; height: max-content;">
        ...
        <span class="no-wrap">manage role</span>
    </a>
</div>
<div class="simple-line-separator mt8"></div>
<div class="mt8 lblack">
    <p class="no-margin bold forum-color">Description</p>
    <span class="fs13">{{ $role->description }}</span>
</div>
<div class="simple-line-separator mt8"></div>
<p class="my8 forum-color bold">Role permissions :</p>
<div class="flex flex-wrap section-style" style="padding: 10px 6px 6px 10px;">
    @if($role->slug == 'site-owner')
        <p class="gray italic blue no-margin mb4">Absolute control.</p>
    @else
        <div class="flex flex-wrap y-auto-overflow" style="max-height: 200px;">
...
<p class="no-margin bold forum-color" style="margin-top: 12px">Members have this role :</p>
<div class="section-style flex flex-wrap my8">
    ...
    <div class="flex align-center">
        ...
        <p class="gray italic my4">This role is not currently acquired by any user yet.</p>
    </div>
    @endif
</div>
    ...
    <div class="my8 mx8">
        <div>
            <div class="flex align-center" style="margin-bottom: 2px">
                <a href="{{ route('forum.all.threads', ['forum'=>$thread['thread']->category->forum->slug]) }}" class="blue no-underline fs11">{{ __($thread['thread']->category->forum->forum) }}</a>
                <svg class="size10 mx4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 512"><path d="M224.31,239l-136-136a23.9,23.9,0,0,0-33.9,0l-22.6,22.6a23.9,23.9,0,0,0,0,33.9l96.3,96.5-96.4,96.4a23.9,23.9,0,0,0,0,33.9L54.31,409a23.9,23.9,0,0,0,33.9,0l136-136a23.93,23.93,0,0,0,.1-34Z"/></svg>
                <a href="{{ $thread['thread']->category->link }}" class="blue no-underline fs11">{{ __($thread['thread']->category->category) }}</a>
            </div>
            <div class="flex">
                <a href="{{ route('user.profile', ['user'=>$thread['thread']->user->username]) }}" class="small-image-3 rounded mr4 hidden-overflow relative has-lazy" style="min-width: 22px">
                    <div class="fade-loading"></div>
                    <img data-src="{{ $thread['thread']->user->sizedavatar(36, '-l') }}" class="lazy-image image-with-fade" alt="">
                </a>
                <div class="full-width">
                    <a href="{{ $thread['thread']->link }}" class="html-entities-decode no-margin bold no-underline forum-color fs13">{{ $thread['thread']->slice }}</a>
                    <div class="flex align-center mt4">
                        <div class="flex align-center">
                            ...
                            <p class="fs11 no-margin">{{ $thread['thread']->view_count }}</p>
                        </div>
                        ...
                        <div class="flex align-center ml8">
                            ...
                            <p class="fs11 no-margin">{{ $thread['postscount'] }}</p>
                        </div>

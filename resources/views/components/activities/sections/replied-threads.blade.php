<div class="activities-section activities-replied-threads-section">
    <div class="activities-section-content-header flex">
        <h3 class="move-to-middle forum-color flex align-center unselectable">
            ...
            {{ __('Replied posts') }} [<span class="current-section-thread-count mx4">{{ $repliedthreads->count() }}</span> / {{ $repliedthreadscount }} ]
        </h3>
    </div>
    <div class="activities-section-content">
        ...
        @endforeach
        @if($repliedthreadscount > 10)
        <div class="flex activity-section-load-more">
            <input type="hidden" class="section" value="replied-threads" autocomplete="off">
            ...
            <div class="full-center" style="margin: 46px auto 0 auto; width: 80%;">
                <div class="flex flex-column align-center">
                    ...
                    @if(auth()->user() && auth()->user()->id == $user->id)
                        <p class="fs20 text-center bold my8 text-center">{{ __("You don't have any replies on posts for the moment") }} !</p>
                        <div class="flex">
                            <div class="flex align-center move-to-middle my4">
                                <p class="text-center no-margin text-center">{{ __("Explore other peoples' posts and share your replies to interact with other people posts") }}</p>
                            </div>
                        </div>
                    @else
                        <p class="fs20 text-center bold my8 text-center">{{ __("This user has no replies on posts for the moment") }} !</p>
                    @endif
                </div>
            </div>
        @endif

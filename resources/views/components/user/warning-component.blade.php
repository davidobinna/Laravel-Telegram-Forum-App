<div class="user-warning-record mb8">
    <input type="hidden" class="warning-id" value="{{ $warning->id }}" autocomplete="off">
    <p class="no-margin lblack"><strong>{{ __('warning') }} :</strong> <span class="warning-name">{{__($warning->reason->name)}}</span></p>
    <div class="relative width-max-content mt2">
        <p class="no-margin fs11 tooltip-section gray">{{__('warned')}} : {{ $warning->athummans }}</p>
        <div class="tooltip tooltip-style-1">
            {{ $warning->warningdate }}
        </div>
    </div>
    <div class="mt4">
        <!-- warned resource is lazy - means it is only available when user click on warned resource button -->
        @switch($warning_resource_type)
            @case('App\Models\Thread')
                <div class="simple-resource-render-box width-max-content mt2 full-width">
                    <div class="flex align-center pointer width-max-content get-ws-simple-resource-render">
                        <span class="block lblack fs12 bold unselectable">{{ __('warned post') }}</span>
                        ...
                        <input type="hidden" class="resource-type" value="{{ $warning_resource_type }}" autocomplete="off">
                        <input type="hidden" class="resource-id" value="{{ $warning->resource_id }}" autocomplete="off">
                        <input type="hidden" class="index" value="{{ $warning->id }}" autocomplete="off">
                    </div>
            ...
            @case('App\Models\Post')
                <div class="simple-resource-render-box width-max-content mt2 full-width">
                    ...
                    <div class="flex align-center pointer width-max-content get-ws-simple-resource-render">
                        <span class="block lblack fs12 bold unselectable">{{ __('warned reply') }}</span>
                        <div class="relative full-center size10 ml4">
                            <svg class="flex icon-above-spinner size6" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 30.02 30.02" style="transform: rotate(0deg);"><path d="M13.4,1.43l9.35,11a4,4,0,0,1,0,5.18l-9.35,11a4,4,0,1,1-6.1-5.18L14.46,15,7.3,6.61a4,4,0,0,1,6.1-5.18Z"></path></svg>
                            <svg class="spinner size10 opacity0 absolute" style="top: 0; left: 0" fill="none" viewBox="0 0 16 16">
                                <circle cx="8" cy="8" r="7" stroke="currentColor" stroke-opacity="0.25" stroke-width="2" vector-effect="non-scaling-stroke"></circle>
            ...
            @case('App\Models\Report')
                <div class="ml8">
                    <span class="block lblack fs13">{{ __('warned report') }} : <strong>{{ __(str_replace('-', ' ', $report->type)) }}</strong></span>
                    <div class="simple-resource-render-box width-max-content mt2 full-width">
                        <div class="flex align-center pointer width-max-content get-ws-simple-resource-render">
            ...
            @case('User\Avatar')
                <div class="toggle-box">
                    <div class="flex align-center pointer toggle-container-button">
                        <span class="fs12 lblack bold">{{__('warned avatar')}} ({{__('click to preview')}})</span>
            ...
            @case('User\Cover')
                <div class="toggle-box">
                    <div class="flex align-center pointer toggle-container-button">
                        <span class="fs12 lblack bold">{{__('warned cover')}} ({{__('click to preview')}})</span>
                        <svg class="toggle-arrow size6 ml4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 30.02 30.02" style="transform: rotate(0deg);"><path d="M13.4,1.43l9.35,11a4,4,0,0,1,0,5.18l-9.35,11a4,4,0,1,1-6.1-5.18L14.46,15,7.3,6.61a4,4,0,0,1,6.1-5.18Z"></path></svg>
                    </div>
    ...
    @if($is_admin)
        ...
        <div class="mt8 typical-button-style flex align-center width-max-content open-warning-remove-from-user-dialog" style="padding: 4px 8px">
            ...
            <span class="fs11 bold">clear warning</span>
            <input type="hidden" class="warning-id" value="{{ $warning->id }}" autocomplete="off">
        </div>
        @else

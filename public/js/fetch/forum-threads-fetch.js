let forum_threads_fetch_more = $('.forum-threads-fetch-more');
let forum_threads_fetch_more_lock = true;
if(forum_threads_fetch_more.length) {
    $(window).on('DOMContentLoaded scroll', function() {
      // We only have to start loading and fetching data when user reach the bottom
      if(forum_threads_fetch_more.isInViewport()) {
        if(!forum_threads_fetch_more_lock) {
            return;
        }
        forum_threads_fetch_more_lock=false;
    
        let forum = $('#forum-id').val();
        let skip = $('.current-threads-count').val();
        let tab = $('.date-tab').val();
    
        let url = `/forums/${forum}/threads/loadmore?skip=${skip}&tab=${tab}`;
        $.ajax({
            type: 'get',
            url: url,
            success: function(response) {
              ...
              if(!response.hasmore) {
                forum_threads_fetch_more.remove();
              }
                ...
                forum_threads_fetch_more_lock = true;
            },
